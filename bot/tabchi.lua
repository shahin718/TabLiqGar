...
if text == 'عضویت فعال' and is_sudo(msg) then

          d:set('joinlink','yes')
         tabchi.sendText(msg.chat_id_, msg.id_, 1,'`عضویت خودکاره فعاله مشتی 😎*', 1, 'md')
                  print("Tabchi [ Message ]")

end
        if text == 'عضویت غیرفعال' and is_sudo(msg) then

          d:set('joinlink','no')
         d:del('joinlink','yes')  
        tabchi.sendText(msg.chat_id_, msg.id_, 1,'`عضویت خودکار غیرفعاله جیگر😍*', 1, 'md')
                print("Tabchi [ Message ]")

  end
if text == 'ذخیره مخاطب فعال and is_sudo(msg) then

          d:set('savecontact','yes')
         tabchi.sendText(msg.chat_id_, msg.id_, 1,'`ذخیره مخاطب فعاله 😐`  *Has Been Enabled*', 1, 'md')
                 print("Tabchi [ Message ]")

 end
        if text == 'ذخیره مخاطب غیرفعال' and is_sudo(msg) then

          d:set('savecontact ','no')
          d:del('savecontact','yes')

          tabchi.sendText(msg.chat_id_, msg.id_, 1,'`ذخیره مخاطب غیرفعاله😐*', 1, 'md')
                  print("Tabchi [ Message ]")
